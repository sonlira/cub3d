...
/*   Updated: 2025/12/02 14:16:20 by bgil-fer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_images(t_game *g)
{
	...
	w = PIXELS;
	h = PIXELS;
	g->n.img = mlx_xpm_file_to_image(g->app->mlx, g->cfg->textures[0], &w, &h);
	g->n.addr = mlx_get_data_addr(g->n.img, &g->n.bpp, &g->n.line_len,
		&g->n.endian);
	g->s.img = mlx_xpm_file_to_image(g->app->mlx, g->cfg->textures[1], &w, &h);
	g->s.addr = mlx_get_data_addr(g->s.img, &g->s.bpp, &g->s.line_len,
		&g->s.endian);
	g->w.img = mlx_xpm_file_to_image(g->app->mlx, g->cfg->textures[2], &w, &h);
	g->w.addr = mlx_get_data_addr(g->w.img, &g->w.bpp, &g->w.line_len,
		&g->w.endian);
	g->e.img = mlx_xpm_file_to_image(g->app->mlx, g->cfg->textures[3], &w, &h);
